...
class AnalyticsSummarySheet implements FromCollection, WithHeadings, WithTitle, WithStyles
{
    ...
    public function collection()
    {
        return collect([
            ...
            // Revenue Analytics
            ['REVENUE ANALYTICS', '', '', ''],
            ['Total Revenue', '$' . number_format($this->revenueStats['total_revenue'], 2), '', ''],
            ['Admin Commission', '$' . number_format($this->revenueStats['total_admin_commission'], 2), '', ''],
            ['Seller Earnings', '$' . number_format($this->revenueStats['total_seller_earnings'], 2), '', ''],
            ['Coupon Discounts', '$' . number_format($this->revenueStats['total_coupon_discounts'], 2), '', ''],
            ['Transaction Count', $this->revenueStats['transaction_count'], '', ''],
            ['Average Order Value', '$' . number_format($this->revenueStats['average_order_value'], 2), '', ''],
            ['', '', '', ''],
            ...
            ['Total Payout Amount', '$' . number_format($this->payoutStats['total_payout_amount'], 2), '', ''],
            ['Pending Payout Amount', '$' . number_format($this->payoutStats['pending_payout_amount'], 2), '', ''],
            ['Average Payout Amount', '$' . number_format($this->payoutStats['average_payout_amount'], 2), '', ''],
            ...
            ['Total Refund Amount', '$' . number_format($this->refundStats['total_refund_amount'], 2), '', ''],
            ['Average Refund Amount', '$' . number_format($this->refundStats['average_refund_amount'], 2), '', ''],
...
class TopSellersSheet implements FromCollection, WithHeadings, WithMapping, WithTitle, WithStyles
{
    ...
    public function map($seller): array
    {
        ...
        return [
            ...
            '$' . number_format($seller->total_earnings, 2),
            $seller->payout_count,
        ];
    }
