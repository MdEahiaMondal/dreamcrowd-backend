...
class PublicWebController extends Controller
{
    ...
    public function Index() {
        ...
        ->get();
        for ($i=1; $i <= 8 ; $i++) { 
            $field = 'expert_link_' . $i;  // Dynamically access the field
            $ids = explode('/', $home->$field);
            if ($ids != null) {
                ${'gig_' . $i}  = TeacherGig::where(['id'=>$ids[1],'status'=>1])->first(); 
                if (${'gig_' . $i}) {
                    ${'profile_' . $i}  = ExpertProfile::where(['user_id'=>${'gig_' . $i}->user_id,'status'=>1])->first(); 
                }else{
                    ${'gig_' . $i}  = null; 
                    ${'profile_' . $i} = null ;
        ...
        return view("Public-site.index", compact('home','home2','categories','gig_1','gig_2','gig_3','gig_4','gig_5','gig_6','gig_7','gig_8','profile_1','profile_2','profile_3','profile_4','profile_5','profile_6','profile_7','profile_8'));
    }
