...
if ($tag) {
    ...
    $gigs = TeacherGig::where('status', 1) 
        ->select('*')
        ->selectRaw('
...
$gigs->getCollection()->transform(function ($gig) use ($wishlist) {
    ...
    if ($recentlyViewedCategory) {
        ...
        return view('Public-site.services', compact('profile','user','faqs', 'recentlyViewedGigs', 'sameCategoryGigs','teacher_services','list','teacher_services_cls','teacher_services_fls')) ;
