...
<html lang="en">
  ...
  @endif
     ...
     <section class="home-section">
          ...
          <div class="col-md-12">
            <div class="dash">
              <div class="row">
                <div class="col-md-12">
                  <div class="dash-top">
                    ...
                    <i class="fa-solid fa-chevron-right"></i>
                    <span class="min-title">Top Seller's</span>
                  </div>
                </div>
              </div>
              ...
              <div class="col-md-12 manage-profile-faq-sec edit-faqs">
                <h5>Edit Services Listing Sortin</h5>
               ...
               <div class="form-section">
                <div class="row">
                  <form action="/update-services-sorting-re" method="POST">
                    @csrf
                    
                    <div class="col-12 mt-0">
                      <label for="inputAddress2" class="form-label">Impressions (%)</label> 
                      <input type="text" class="form-control" value="@if($tag){{$tag->sorting_impressions}}@endif"
                             id="impressions" name="impressions" placeholder="%" required />
                    </div>
                    
                    <div class="col-12 mt-0">
                      <label for="inputAddress2" class="form-label">Clicks (%)</label>
                      <input type="text" class="form-control" value="@if($tag){{$tag->sorting_clicks}}@endif"
                             id="clicks" name="clicks" placeholder="%" required />
                    </div>
                    
                    <div class="col-12 mt-0">
                      <label for="inputAddress2" class="form-label">Orders (%)</label>
                      <input type="text" class="form-control" value="@if($tag){{$tag->sorting_orders}}@endif"
                             id="orders" name="orders" placeholder="%" required />
                    </div>
                    
                    <div class="col-12 mt-0">
                      <label for="inputAddress2" class="form-label">Reviews (%)</label>
                      <input type="text" class="form-control" value="@if($tag){{$tag->sorting_reviews}}@endif"
                             id="reviews" name="reviews" placeholder="%" required />
                    </div>
      ...
      {{-- 100% All plus values allowed script Start--}}
      <script>
        const inputs = ['impressions', 'clicks', 'orders', 'reviews'];
        ...
        inputs.forEach(input => {
          document.getElementById(input).addEventListener('input', function(e) {
            ...
            validateTotalPercentage(input_v);
          });
        });
      
        function validateTotalPercentage(input_v) {
          let total = 0;
      
          // Calculate the total percentage of all inputs
          inputs.forEach(input => {
            const value = parseInt(document.getElementById(input).value) || 0;
            total += value;
          });
      ...
      {{-- 100% All plus values allowed script END--}}
...
<script>
  var earning = '<?php echo addslashes($tag->earning); ?>' ;
  var booking = '<?php echo addslashes($tag->booking); ?>' ;
  var review = '<?php echo addslashes($tag->review); ?>' ;
console.log(review);

  // $('#'+earning).val(Number(earning));
  // $('#'+booking).val(Number(booking));
  // $('#'+review).val(Number(review));
</script>
